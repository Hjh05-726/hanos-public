...
当前本地候选版本：**0.1.0-preview.4**。需要帮助时查看[排障与卸载](docs/troubleshooting.md)；参与开发和了解后续范围可查看[维护路线](docs/roadmap.md)。
...
下面的录屏与截图来自实际运行的桌面 HTML 页面：3 个分类、12 篇笔记、51 条联系，内容全部来自虚构演示知识库。

[![HanOS 实际操作演示：缩放与拖拽星图，打开笔记，再返回图谱](docs/assets/atlas-demo.gif)](docs/assets/atlas-demo.mp4)

<sub>8 秒真实操作录屏：缩放、拖拽、关联高亮与同页阅读。动图为压缩预览。</sub>

[查看或下载清晰版 MP4（约 1.2 MB）](docs/assets/atlas-demo.mp4)

<details>
<summary>查看静态界面截图</summary>


![HanOS 真实界面：虚构知识库的横向星图总览](docs/assets/atlas-overview.png)

<sub>完整页面截图：搜索入口、横向知识星图和缩放工具。</sub>

![HanOS 真实界面：聚焦播种计划后，其直接关联节点和连线高亮](docs/assets/atlas-connections.png)

<sub>从演示录屏截取的真实画面：聚焦“播种计划”，查看它与其他笔记和主题的直接联系。</sub>
...
**星图模板严格固定。** 所有 Agent 和模型只能调用包内的统一生成器，不得仿照截图重写页面或修改产物。`template-lock.json` 固定生成器与布局模块的 SHA-256；安装、打包和生成都会校验，缺文件或发生漂移时直接失败。生成后还会逐字节复核 HTML，只有 `HANOS_HTML_VERIFY=PASS` 才是通过验收的标准产物。模板升级须明确修改共享版本并重新验证，普通使用不得自行换皮。
...
通过自然语言读取、导入、记录与纠正时，Skill 现在调用安装内的 `knowledge.py`：范围内查询给出原文、位置与读取摘要；资料导入保全 Markdown/文本原始字节和覆盖报告；修改绑定读取版本并保留可检查的恢复记录。日常仍只捕获最小长期信息，不全文归档对话。现有 Markdown 无需补字段或迁移，锁定星图保持原样。

详见[查询支持范围](docs/knowledge-query.md)、[操作与恢复](docs/knowledge-operations.md)。新工具写入当前支持 macOS/Linux；多文件操作不是数据库事务，程序位置核验不能替代来源语义审阅。`0.1.0-preview.4` 为本地候选，不表示已经远端发布。
